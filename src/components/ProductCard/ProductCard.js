import React from "react";
import '../../components/ProductCard/ProductCard.css';


const ProductCard = (props) => {

    return (
        props.items.map(products =>(<div className="card-typ-vertical">
            <div className="card">
                <div className="card-typ-1">
                    <div className="card-img-vertical-container">
                        <div className="badge-bs">#1 Bestseller</div>
                        <img
                            src="assests/images/iphone-baner.png"
                            alt="mobile stock"
                        />
                    </div>
                    <div className="card-body">
                        <h5 className="card-h-style f-s-h5">
                            {products.title}
                        </h5>
                        <p className="card-text-paragraph text-subtitle-color">
                            {products.categoryName}
                        </p>
                    </div>
                    <div className="card-footer">
                        <div className="card-info">
                            <div className = "price-info">
                                <span className="price-after">{products.price}</span>
                                <span className="price-before text-subtitle-color">
                                    ₹100
                                </span>
                                <span className="price-percent-difference">
                                    1% Off</span>
                            </div>
                            <div className="rating-info">
                                <span className="star-rating">4.2⭐</span>
                            </div>
                        </div>
                        <div className="card-buttons">
                            <span className="btn-add-to-card">
                                <button className="btn-1 is-primary">
                                    <i className="fa-solid fa-cart-shopping"></i>
                                    Add To Cart
                                </button>
                            </span>
                            <span className="btn-wishlist">
                                <button className="btn-1 bg-none">
                                    <i className="fa fa-heart-o"></i>
                                </button>
                            </span>
                        </div>
                    </div>
                </div>
            </div>
        </div>))
    );

};

export default ProductCard;